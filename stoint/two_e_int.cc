#include "twoeints/two_e_tables.h"
#include "stoint.h"
#include <tgmath.h>
#include "auxiliary.h"
#include <csignal>
...
void checkq(quartet q) {
    checko(q[0]); checko(q[1]); checko(q[2]); checko(q[3]);
}
...
double two_e_int(quartet q) {
    ...
    return (*f)(q[0].z+q[1].z,q[2].z+q[3].z) * QNormCoef(q[0].n,q[1].n,q[2].n,q[3].n,q[0].z,q[1].z,q[2].z,q[3].z);
}
